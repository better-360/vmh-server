...
export class NotificationListener implements OnModuleInit {
  ...
  onModuleInit() {
   ...
   async sendWelcomeMail(payload: { email: string; name: string }) {
    this.mailService.sendWelcomeMail(payload.email, payload.name);
   }
  ...
  async sendnewDocumentMail(payload: {
    ...
    this.mailService.sendnewDocumentMail(
      payload.email,
      payload.name,
  ...
  async sendEmailVerifyMail(payload: {
    ...
    this.mailService.sendEmailVerifyMail(
      payload.email,
      payload.fullName,
      payload.verifyLink,
    );
  ...
  async sendTicketResolvedMail(payload: {
    ...
    this.mailService.sendTicketResolvedMail(
      payload.email,
      payload.fullName,
