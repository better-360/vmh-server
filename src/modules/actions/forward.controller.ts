...
export class ForwardController {
  ...
  @Post('debug/rates')
  @ApiOperation({ summary: 'Debug endpoint to test EasyPost rates' })
  @ApiResponse({ status: 200, description: 'Debug information about rates' })
  async debugRates(@Body() dto: {
    fromAddress: any;
    toAddress: any;
    parcel: any;
  }) {
    // Bu endpoint sadece development için - production'da kaldırılabilir
    try {
      const { fromAddress, toAddress, parcel } = dto;
      
      console.log('Debug rates request:', { fromAddress, toAddress, parcel });
      
      // EasyPost service'i doğrudan çağır
      const rates = await this.forwardService['easyPostService'].getAllRates(
        toAddress, 
        fromAddress, 
        parcel
      );
  ...
  @Post('debug/mail-data')
  @ApiOperation({ summary: 'Debug endpoint to check mail and address data' })
  @ApiResponse({ status: 200, description: 'Mail and address debug information' })
  async debugMailData(@Body() dto: {
    mailId: string;
    deliveryAddressId: string;
  }) {
    try {
      // Mail ve ilgili bilgileri getir
      const mail = await this.forwardService['prisma'].mail.findUnique({
        where: { id: dto.mailId },
        include: {
          mailbox: {
            include: {
      ...
      const deliveryAddress = await this.forwardService['prisma'].deliveryAddress.findUnique({
      ...
      const parcel = {
        length: mail.length || 1,
        width: mail.width || 1,
        height: mail.height || 1,
        weight: mail.weight || 1,
      };
